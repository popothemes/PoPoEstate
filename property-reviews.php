...
?>
<div class="p-box">
<h4><?php echo __("Reviews", 'poporealestate'); ?></h4>
...
<div id="comments" class="comments-area">
    ...
    if (1 === $comments_number ) {
        ...
        printf(_x('One review', 'reviews title', 'poporealestate'));
    } else {
        printf(
    ...
    if (! comments_open() && get_comments_number() && post_type_supports(get_post_type(), 'comments') ) :
        ?>
      <p class="no-comments"><?php _e('Reviews are disabled.', 'poporealestate'); ?></p>
    <?php endif; ?>
    ...
    comment_form(
        array(
            ...
            'class_submit'      => 'submit',
            'name_submit'       => 'submit',
            'title_reply'       => __( 'Leave a Review', 'poporealestate' ),
            'title_reply_to'    => __( 'Leave a Reply to %s', 'poporealestate'  ),
            'cancel_reply_link' => __( 'Cancel Review', 'poporealestate'  ),
            'label_submit'      => __( 'Post Review', 'poporealestate' ),
            'format'            => 'xhtml',

            'comment_field' =>  '<p class="comment-form-comment"><label for="comment">' . __( 'Your Review', 'poporealestate' ) .
                '</label><textarea id="comment" class="form-control" placeholder="'.__('Your Review', 'poporealestate').'" name="comment" rows="5" aria-required="true">' .
                '</textarea></p>',
